...
//                                      ----------------------------------
//
#define CNF_BOARD_MODEL 21  // Mettre la valeur correspondante au tableau ci-dessous !
//                         // 
...
/**
** @@RC type ARDUINO IDE BOARD TYPE : NodeMCU 1.0 (ESP12-E Module)
**/
#include "CustomDef.h"
bool RFGroup = false;
byte RFOnOff = 0;
...
//
 ...
 ligne à supprimer apres avoir modifié ssid et password
    ...
    #endif
    //
    IPAddress CNF_IP_JEEDOM (192, 168, 1, 7); // ADRESSE IP du serveur JeeDom
    //
    IPAddress CNF_IP_ARDUIN (192, 168, 1, 32); // ADRESSE IP DE L'ARDUINO
    //                /!\ l'IP est en DHCP sur WeMOS /!\
    //          il vous faudra creer une reservation d'IP sur votre routeur pour eviter les soucis !
    //
    IPAddress CNF_IP_GATEWAY (192, 168, 1, 1);
	...
	for (byte td= 0; td < CNF_NB_DPIN; td++) {
      ...
      for (int i = 2; i < CNF_NB_DPIN + CNF_NB_APIN + CNF_NB_CPIN; i++) {
        ...
        if (EEPROM.read(i) == 'z' || EEPROM.read(i) == '\0') {
        	...
        	// @@RC FIX17 the max of i is CNF_NB_DPIN + CNF_NB_APIN + CNF_NB_CPIN
        	//  the size of TimerDelays is CNF_NB_DPIN :( 
        	// initialialisation done in header of the function
